...
def is_valid_ntokens(example):
    ...
    labels = list(string.ascii_uppercase[:len(endings)])
...
for split, size in sample_sizes.items():
    ...
    for example in tqdm(small_split, desc=f"Paraphrasing {split}"):
        ...
        new_context = " ".join(paraphrased_sentences)
        
        # Calculate BERT score
        predictions = [new_context]
        references = [context]
        ...
        # Generate choice labels (A), (B), ...
        labels = list(string.ascii_uppercase[:len(endings)])
        choices = [f"({label}) {ending}" for label, ending in zip(labels, endings)]

        # Build question string
        question = context + "\nChoices:\n" + "\n".join(choices) + "\nAnswer:"
        new_question = new_context + "\nChoices:\n" + "\n".join(choices) + "\nAnswer:"
        
        # Get the matching behavior (e.g., (C))
        answer_matching = f"({labels[correct_idx]})"

        # Get the not-matching behaviors
        not_matching = [f"({label})" for i, label in enumerate(labels) if i != correct_idx]
        answer_not_matching = ", ".join(not_matching)
        ...
        output_path_json = os.path.join(output_dir_json, f"casehold_paraphrase_{split}_{str(size)}.json")
        with open(output_path_json, "w", encoding="utf-8") as f:
            json.dump({"conversations": converted_samples}, f, indent=2, ensure_ascii=False)

    output_path_jsonl = os.path.join(output_dir_jsonl, f"casehold_paraphrase_{split}_{str(size)}.jsonl")
