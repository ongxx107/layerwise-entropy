...
for split, size in sample_sizes.items():
    ...
    for example in tqdm(small_split, desc=f"Paraphrasing {split}"):
        ...
        output_path_json = os.path.join(output_dir_json, f"ecthr_a_paraphrase_{split}_{str(size)}.json")
        with open(output_path_json, "w", encoding="utf-8") as f:
            json.dump({"conversations": converted_samples}, f, indent=2, ensure_ascii=False)

    output_path_jsonl = os.path.join(output_dir_jsonl, f"ecthr_a_paraphrase_{split}_{str(size)}.jsonl")
